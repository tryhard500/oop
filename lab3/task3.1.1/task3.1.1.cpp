...
public:
    ...
    Iterator begin() {
        int index = 0;
        while (index < capacity && !table[index]) ++index;
...
template<typename K, typename V> class MultiHashMap {
    struct HashMapNode {
        K key;
        V value;
        HashMapNode *next;
...
public:
    MultiHashMap(float loadFactor = 0.70, int capacity = 4) :
        loadFactor(loadFactor), capacity(capacity), size(0), table(new HashMapNode *[capacity]()) {}
...
std::pair<int, int> executeProgram(char keyType, char valueType, std::ifstream &file) {
    switch (keyType) {
        ...
        case 'M':
            switch (valueType) {
                case 'I': return makeHashMap<Matrix, int>(file);
                case 'D': return makeHashMap<Matrix, double>(file);
                case 'S': return makeHashMap<Matrix, std::string>(file);
                default: throw std::out_of_range("Wrong value type");
            }
...
int main() {
    ...
    std::cout << std::endl << map.get("A") << std::endl;
